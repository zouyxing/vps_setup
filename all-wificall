...
if [ "$PACKAGE_MANAGER" = "apt" ]; then
    ...
    apk add iptables ufw expect curl wget iproute2
    # Alpine 使用 OpenRC 或 BusyBox init，但 systemd 相关的服务脚本我们保留，
    # 它们只会在 systemd 系统上启用。
fi
...
EOF
    ...
    # 针对非 SystemD 系统 (如 Alpine with OpenRC)，此处通常需要用户手动配置启动脚本
...
# 注意：Alpine Linux可能需要使用 rc-status/service
if command -v systemctl &> /dev/null; then
    echo "  systemctl status xray         # 查看 Xray 运行状态"
else
    echo "  service xray status 或 rc-service xray status # 查看 Xray 运行状态 (非SystemD)"
fi
echo ""
